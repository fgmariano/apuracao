create procedure sp_bateria(@escola int, @nota decimal(4,1), @posNota int)
as
	declare @maior decimal(4,1), @menor decimal(4,1), @total decimal(4,1)
	if (@posNota = 1) begin
		insert into bateria (id_escola, nota1) values
		(@escola, @nota)
	end
		...
		update bateria
			...
			where id_escola = @escola
		update bateria
			set maior = dbo.fn_maior(3, @escola), menor = dbo.fn_menor(3, @escola)
			where id_escola = @escola
		select @total = dbo.fn_total(@escola, 3)
		update bateria
			set total = @total
			where id_escola = @escola
