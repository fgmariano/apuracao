create procedure sp_enredo(@escola int, @nota decimal(4,1), @posNota int)
as
	declare @maior decimal(4,1), @menor decimal(4,1), @total decimal(4,1)
	if (@posNota = 1) begin
		insert into enredo (id_escola, nota1) values
		(@escola, @nota)
	end
		...
		update enredo
			...
			where id_escola = @escola
		update enredo
			set maior = dbo.fn_maior(8, @escola), menor = dbo.fn_menor(8, @escola)
			where id_escola = @escola
		select @total = dbo.fn_total(@escola, 8)
		update enredo
			set total = @total
			where id_escola = @escola
