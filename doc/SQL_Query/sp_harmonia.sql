create procedure sp_harmonia(@escola int, @nota decimal(4,1), @posNota int)
as
	declare @maior decimal(4,1), @menor decimal(4,1), @total decimal(4,1)
	if (@posNota = 1) begin
		insert into harmonia (id_escola, nota1) values
		(@escola, @nota)
	end
		...
		update harmonia
			...
			where id_escola = @escola
		update harmonia
			set maior = dbo.fn_maior(5, @escola), menor = dbo.fn_menor(5, @escola)
			where id_escola = @escola
		select @total = dbo.fn_total(@escola, 5)
		update harmonia
			set total = @total
			where id_escola = @escola
